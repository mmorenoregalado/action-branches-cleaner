...
export class CleanerBranches {
  ...
  async run({ignoredBranches}: {ignoredBranches: string[]}): Promise<void> {
    ...
    const mergedBranches = this.repository.mergedBranches(
      branches,
      pullRequests
    )
    const filteredBranches = this.filterWithoutNullsAndIgnoredBranches(
      mergedBranches,
      ignoredBranches
    )
  ...
  private filterWithoutNullsAndIgnoredBranches(
    branches: Branch[],
